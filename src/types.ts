...
export type IAirtableService = {
	...
	listWebhooks(baseId: string): Promise<{webhooks: Array<Record<string, unknown>>}>;
	createWebhook(baseId: string, notificationUrl: string, specification: Record<string, unknown>): Promise<Record<string, unknown>>;
	deleteWebhook(baseId: string, webhookId: string): Promise<void>;
	getWebhookPayloads(baseId: string, webhookId: string, cursor?: number | undefined): Promise<{payloads: unknown[]; cursor: number; mightHaveMore: boolean}>;
	refreshWebhook(baseId: string, webhookId: string): Promise<{expirationTime: string | null}>;
};
